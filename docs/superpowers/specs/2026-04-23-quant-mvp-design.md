...
1. Install `pq` CLI on a laptop
2. `pq auth login` (optional; unlocks telemetry and the hosted-tenant funnel)
3. `pq new strategy hello-world` — scaffold a volatility-forecasting strategy
4. `pq up` — start the local stack via docker-compose
5. `pq run hello-world` — train + walk-forward + PBO/DSR/CPCV gate; passes
...
9. Also available: `pq new strategy hello-world-returns` — companion template that demonstrably fails the gate, teaching users the promotion is real
...
- `pq ai <prompt>` — Claude Agent SDK integration (`pq ai explain run`, `pq ai suggest-threshold`, `pq ai new-strategy`).
- `pq data register <name>` — bring-your-own-data registration flow. MVP uses bundled data only.
- `pq deploy` — push to tenant GCP project. The image is built by CI, but the deploy path is not exercised in MVP.
- `pq submit` — scheduled / background runs via PGMQ. MVP uses interactive `pq run` only.
- `pq watch` — file-watch hot reload.
- `pq promote` as CLI — promotion is UI-only for MVP (CLI gets the verb in v2).
...
| 6 | Model registry | MLflow 2.16+ with Aliases only; `.stage` accessor banned via `pq check` AST walk | MVP-A's Aliases/Stages drift failure must not recur; CI-enforced. |
...
| 9 | Auth | BFF + mock OIDC for local UI; GitHub OAuth device flow for `pq auth login` (hosted accounts service) | Local: no friction. Hosted: zero-infra device flow. |
...
| 15 | Testing | Five layers: unit, integration (testcontainers), E2E (`pq e2e`), UI (Playwright), HIL (human-scripted) | MVP-A had rigorous TDD at wrong level; HIL is the missing layer. |
...
### 5.1 Local topology (MVP demo, `pq up` on laptop)

```
┌─ Host (quant's laptop) ────────────────────────────────────────────────┐
│  pq CLI (`~/.local/bin/pq` via uv tool install)                        │
│  Python venv for the strategy project (uv-managed, per-project)        │
│  Strategy code runs here by default: `pq run hello-world`              │
│  IDE (VS Code / PyCharm) with scaffolded debug configs                 │
│                                                                        │
│          │ HTTP                                                        │
│          ▼                                                             │
└────────────────────────────────────────────────────────────────────────┘
┌─ docker-compose network (`pq up`) ─────────────────────────────────────┐
│                                                                        │
│   ┌───────────────┐   ┌─────────────┐   ┌─────────────────────────┐    │
...
│   │   (Also handles WORKER_ROLE=training for `pq e2e` container  │     │
│   │    mode and pre-push; not used by default `pq run`.)         │     │
│   └──────────────────────────────────────────────────────────────┘     │
└────────────────────────────────────────────────────────────────────────┘
                          │
                          │ (only when `pq auth login` or telemetry flows)
                          ▼
...
- **Default `pq run`**: strategy runs as a subprocess on the host, in the project's uv venv. Native IDE debugging. Zero container round-trip.
- **`pq run --container` or `pq e2e`**: strategy runs inside the worker container (WORKER_ROLE=training), with the project bind-mounted at `/workspace`. `debugpy` listening on `:5678` for remote attach. Used for drift-catching and pre-push gates.
...
Acknowledged tradeoff: the worker container in MVP mostly serves inference (the PGMQ+worker orchestration pattern is provisioned but not heavily exercised). The seat is built for future `pq submit`, scheduled runs, and training-in-worker when a user opts in via `--container`. MVP deliberately under-uses it rather than over-engineering it. This is a conscious, recorded decision — not an oversight.
...
- `pq` — the CLI (exposed via `[project.scripts]`)

Installation paths:
- `uv tool install quantplatform   # puts pq on PATH` (Python-native)
...
Key SDK guarantees:
- `data.ohlcv(...)` / `data.get(...)` are the ONLY sanctioned data access paths; direct `pl.read_*` / `pd.read_*` / `open()` / `requests.get()` in strategy code are flagged by `pq check`.
...
| Command | Purpose |
|---|---|
| `pq auth login` | GitHub device flow; stores `~/.pq/credentials`; one-time telemetry opt-in prompt |
| `pq new strategy <name>` | Scaffold a project from template (V by default; R via `--template returns`) |
| `pq up` | Start the local stack (wraps `docker compose up -d`) |
| `pq down` | Stop the local stack |
| `pq doctor` | Verify Docker, uv, Python version, free ports, project-venv health |
| `pq run <strategy>` | Execute strategy (host by default; `--container` for container-run; `--debug` to wait for IDE attach) |

Deferred verbs (post-MVP): `pq e2e` (used internally by pre-push hook but not a public verb in MVP), `pq promote`, `pq logs`, `pq test`, `pq check`, `pq ai`, `pq data register`, `pq watch`, `pq submit`, `pq deploy`.

**`pq check`** is used internally during `pq new` scaffolding and in pre-commit to AST-walk strategies, but is not exposed as a public command in MVP CLI surface. ruff and pyright remain user-invoked via `uv run`.
...
│   └── conftest.py
├── pq.toml                            # name, entry, thresholds, data deps
...
├── .pre-commit-config.yaml            # ruff + pyright + pq-check + unit tests
├── .githooks/pre-push                 # pq e2e (container build + run)
...
- **`SERVICE_ROLE=worker_training`** — Used only for `pq run --container` and `pq e2e`; reads from PGMQ training queue and executes strategy code with bind-mounted project at `/workspace`.
...
1. **Login** — mock OIDC in local mode; session cookie writeable by `pq up --open-browser`
...
- `POST /events` — telemetry ingest (anonymous command, duration, success, pq version; never arguments, never strategy code, never data)

Opt-out: `QP_TELEMETRY=0` env var, `DO_NOT_TRACK=1` env var, or `pq auth logout`.
...
- `pq check` AST-walk bans `.stage` / `transition_model_version_stage` across the entire codebase
...
Strategies are Python packages in the user's project directory. `pq new strategy` scaffolds. `pq up` bind-mounts the project into the worker container (for container mode) or leaves it alone (for host mode).

Registration flow (no user-called `register()` function; the platform handles it):
1. `pq run <name>` reads `pq.toml` for `name`, `entry`, and thresholds
2. Before executing, the CLI imports the Strategy class (via the declared entry point) and confirms `Strategy.name` matches `pq.toml`
...
Drift-catching: pre-push hook runs `pq e2e` — builds the worker image, runs the strategy inside the container with `debugpy` optionally available, asserts same-outcome. Budget: <90 seconds with BuildKit layer cache.
...
| E2E (`pq e2e`) | Host↔container drift; image-build correctness | docker build + container strategy run | <90s | Pre-push (required); CI main |
...
**Deliverable:** Monorepo skeleton; docker-compose with Postgres + MinIO + MLflow + FastAPI placeholder + mock OIDC; Alembic initialized; `pq up` works.
...
**HIL script (15 min):** Fresh-clone `pq up`; visit all service ports; `pq down` then `pq up` again; state persists; `pq doctor` output reviewed.
...
**Deliverable:** SDK module with `Strategy`, `sdk.data.ohlcv`, `sdk.run.start`, content-hashed lineage writes. `pq new strategy hello-world` scaffolds V template. `pq run hello-world` executes on host, logs to MLflow, writes `runs` + `events` + `lineage_reads`. NO gate yet — runs finish without pass/fail decisions.

**Automated tests:**
- Unit: SDK method contracts; lineage record shape; scaffold correctness
- Integration: `pq run` against live stack; MLflow run created; lineage rows written; audit log extended
- E2E: `pq e2e` container build + run passes in <90s

**HIL script (30 min):** Scaffold, run, browse MLflow UI, inspect Postgres lineage, attach debugger, `pq doctor`.
...
**Automated tests:**
- Unit: gate decision logic; promote handler refuses alias flip on fail; `pq check` AST walk catches bypass attempts
...
**Deliverable:** Login, strategies list, runs list, run-detail with walk-forward chart + PBO/DSR/CPCV, Promote button, unified session via `pq up --open-browser`.
...
### M7 — Accounts service + `pq auth login` (target: 3 workdays)

**Deliverable:** `accounts-api` deployed to ubuntu-server behind Cloudflare Tunnel; GitHub OAuth device flow; `/me`, `/events`; `pq auth login` integration; telemetry client with opt-out.
...
**HIL script (30 min):** Login flow; status check; telemetry lands; opt-out respected; simulate accounts outage; inspect `~/.pq/credentials` for leaks.

**DoD:** Auth flows work; telemetry is opt-out-respecting; offline mode functions.

### M8 — Packaging + demo rehearsal (target: 2 workdays)

**Deliverable:** `quantplatform` on PyPI (or TestPyPI); `uv tool install quantplatform   # puts pq on PATH` verified; `get.quantplatform.io/install.sh` hosted on ubuntu-server; README quickstart; 5-minute demo video.

**Automated tests:**
- Integration: clean install on matrix (macOS, Linux, Python 3.11/3.12/3.13)
- E2E: cold-install VM test — `curl | sh`, `pq new`, `pq up`, `pq run`, expected output
...
| R3 | Host-run strategy passes but container-run fails | `pq e2e` in pre-push; M3 HIL explicitly verifies container mode |
...
| R7 | MLflow `.stage` sneaks into code via copy-paste | `pq check` AST walk in pre-commit; CI grep on main |
| R8 | Strategy code bypasses SDK data access → no lineage | `pq check` AST walk flags `pl.read_*`, `pd.read_*`, `open()`, `requests.get()` |
...
- **`pq ai` (Claude Agent SDK)** — `pq ai explain run <id>`, `pq ai suggest-threshold`, `pq ai new-strategy "mean reversion on SPY"`
- **`pq data register`** — BYO-data flow for customer-owned datasets
- **`pq deploy`** — push tenant image to GCP project
- **`pq submit`** — scheduled / background runs via PGMQ
- **`pq watch`** — file-watch hot reload
